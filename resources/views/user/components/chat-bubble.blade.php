    ...
    <div id="chatBox" class="hidden w-[360px] max-h-[520px] rounded-xl shadow-2xl bg-white absolute bottom-20 right-0 overflow-hidden border border-gray-200 flex flex-col">
        ...
        <div class="bg-[#FF7A00] text-white p-4 flex justify-between items-center">
            <div class="flex items-center gap-3">
                <img id="productImage" src="" alt="Gambar Produk" class="w-10 h-10 rounded-full object-cover">
        ...
        <!-- Messages -->
        <div id="chatMessages" 
            class="h-[300px] overflow-y-auto p-4 space-y-3 text-sm text-gray-800 flex-1" 
                ...
                <div class="flex @if($chat->user_id === auth()->id()) justify-end @else items-start gap-2 @endif">
                    @if($chat->user_id !== auth()->id())
                        <div class="w-8 h-8 bg-gray-300 rounded-full">
                            <img src="{{ $chat->user->avatar_url ?? asset('images/default-profile.jpg') }}" class=" w-full h-full object-cover">
                        </div>
                    @endif
                    <div class="@if($chat->user_id === auth()->id()) bg-[#FF7A00] text-white @else bg-gray-200 @endif px-4 py-2 rounded-lg max-w-[70%]">
                        {{ $chat->message }}
                        <div class="text-xs mt-1 @if($chat->user_id === auth()->id()) text-orange-100 @else text-gray-500 @endif">
            ...
            .then(data => {
                ...
                chats.forEach(chat => {
                    ...
                    msgDiv.innerHTML = `
                        ...
                        <div class="w-8 h-8 bg-gray-300 rounded-full">
                            <img src="${chat.user.avatar_url || '/images/default-profile.jpg'}" class="w-full h-full object-cover">
                        </div>` : ''}
